...
for ipfscid in ${ipfscids[@]}; do
  ...
  ununifid tx nftmint create-class example_s${ucounter} $baseuri 4 0 --symbol unexs${ucounter} --class-uri $classuri --description="this is example nft collection for ununifi" --chain-id=$chainid --from=my_validator --keyring-backend=test --gas=300000 -y --broadcast-mode=block
